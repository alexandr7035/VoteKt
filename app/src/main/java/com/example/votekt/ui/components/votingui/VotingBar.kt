package com.example.votekt.ui.components.votingui
...
private const val VOTING_BAR_ANIMATION_DURATION_MS = 500
private const val VOTING_BAR_STARTING_ANGLE = 270
private const val VOTING_BAR_STARTING_ANGLE_DIVIDER = 4
private const val VOTING_BAR_MAX_ANGLE = 360
...
private fun VotingCircle_Voted(
    ...
    val spaceAngle = if (!shouldHideSpacings(params)) {
        VOTING_BAR_MAX_ANGLE * params.circleSpacingPerc
    } else {
        0f
    }

    var startAngle = VOTING_BAR_STARTING_ANGLE + spaceAngle / VOTING_BAR_STARTING_ANGLE_DIVIDER

    val totalAngle = VOTING_BAR_MAX_ANGLE - spaceAngle
    ...
    LaunchedEffect(Unit) {
        againstArcAnimated.animateTo(
            ...
            animationSpec = tween(durationMillis = VOTING_BAR_ANIMATION_DURATION_MS)
        )
    }
    ...
    VotesComponent(totalVotes, params)
}

@Composable
private fun VotesComponent(totalVotes: Int, params: VotingBarParams) {
    Column {
        val icon: Painter
        val color: Color
        val perc: Float
...
private object VotingColors {
    val forColor = SemanticColors.AppPositiveColor
    val againstColor = SemanticColors.AppNegativeColor
}
