...
import pm.gnosis.crypto.utils.bigInt
import pm.gnosis.crypto.utils.hash160
import pm.gnosis.crypto.utils.toBytes
...
class HDNode(val keyPair: KeyPair, val chainCode: ByteString, val depth: Int, val index: Long, val parentFingerprint: ByteString) {
    ...
    fun deriveChild(index: Long): HDNode {
        ...
        return try {
            ...
            HDNode(
                generateKeyPair(
                    pkn = bytes.substring(0, 32).bigInt(),
                    ccn = keyPair.privKey
                ), newChainCode, depth + 1, index, fingerprint().substring(0, 4)
            )
        } catch (e: Exception) {
            deriveChild(index + 1)
        }
