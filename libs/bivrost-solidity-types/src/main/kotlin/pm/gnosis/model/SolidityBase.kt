...
object SolidityBase {
    ...
    class Vector<out T : Type>(items: List<T>) : Collection<T>(items), DynamicType {
        ...
        class Decoder<out T : Type>(private val itemDecoder: TypeDecoder<T>) : TypeDecoder<Vector<T>> {
            ...
            override fun decode(source: PartitionData): Vector<T> {
                val capacity = decodeUInt(source.consume()).safeIntValueExact()
    ...
    fun decodeBytes(source: PartitionData): ByteArray {
        val contentSize = BigDecimal(BigInteger(source.consume(), 16)).safeIntValueExact() * 2
    ...
    fun <T : Any> decodeArray(data: String, itemDecoder: (String) -> T): List<T> {
        ...
        val contentSize = BigInteger(params.consume()).toLong().toInt()
        if (contentSize == 0) return emptyList()
        return (0 until contentSize).map { itemDecoder(params.consume()) }
    }
}

fun BigInteger.safeIntValueExact(): Int {
    if (this > BigInteger.valueOf(Int.MAX_VALUE.toLong()) || this < BigInteger.valueOf(Int.MIN_VALUE.toLong())) {
        throw ArithmeticException("BigInteger out of int range")
    }
    return this.toInt()
}

// Make it work below android 11
fun BigDecimal.safeIntValueExact(): Int {
    val bigIntegerValue = this.toBigIntegerExact()
    if (bigIntegerValue > BigInteger.valueOf(Int.MAX_VALUE.toLong()) || bigIntegerValue < BigInteger.valueOf(Int.MIN_VALUE.toLong())) {
        throw ArithmeticException("BigDecimal out of int range")
    }
